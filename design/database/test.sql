...
(
	...
	`user_image_url` VARCHAR(255) COMMENT '用户图像url',
...
(
	...
	`visibility` VARCHAR(32) COMMENT '可见范围',
	`title` VARCHAR(32) COMMENT '日常活动标题',
...
(
	...
	`org_announce` VARCHAR(100) COMMENT '组织公告',
	`valid` VARCHAR(3) NOT NULL default 'Y' COMMENT '是否有效',
	`user_id` VARCHAR(32) COMMENT '用户ID',
	`create_time` DATETIME(0) COMMENT '创建时间',
...
(
	...
	`from_name` VARCHAR(32) COMMENT '发送者名称',
...
(
	...
	`file_size` NUMERIC(10,2) COMMENT '文件大小(byte)',
...
(
	...
	`leaf` VARCHAR(2) default 'N' COMMENT '是否为叶子节点',
	`available` VARCHAR(2) default 'Y' COMMENT '是否可用',
...
ALTER TABLE `user_org` 
 ADD INDEX `IXFK_user_org_org_role` (`role_id` ASC)
;
...
ALTER TABLE `banner` 
 ADD INDEX `IXFK_banner_routine_02` (`routine_id` ASC)
;
...
ALTER TABLE `work_group_member` 
 ADD CONSTRAINT `FK_work_group_member_chat_group`
	FOREIGN KEY (`chat_group_id`) REFERENCES `chat_group` (`group_id`) ON DELETE Restrict ON UPDATE Restrict
;
...
ALTER TABLE `banner` 
 ADD CONSTRAINT `FK_banner_routine`
	FOREIGN KEY (`routine_id`) REFERENCES `routine` (`routine_id`) ON DELETE Restrict ON UPDATE Restrict
;
